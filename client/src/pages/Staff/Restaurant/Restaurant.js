    ...
    const fetchData = async () => {
      try {
        const responseRestaurant = await axios.get(
          `${process.env.REACT_APP_API_URL}/staff/restaurant`,
        ...
        if (error.response) {
          console.log("Backend response:", error.response); // debug toàn bộ response
          ...
          if (error.response.data && typeof error.response.data === "object") {
            ...
            toast.error(`Error from server: ${errorMsg}`);
          } else {
  ...
  const searchAvailableTables = async (e) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/restaurant/${restaurant.Id}/available-tables`,
        {
          params: {
            date: selectedDate,
  ...
  return (
    <div className="container-fluid mt-5">
      ...
      <div className="row justify-content-center">
        {/* Sidebar đặt bàn */}
        <div className="col-md-3 p-4 shadow-lg sidebar mt-5">
