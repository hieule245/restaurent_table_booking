...
const Admin = () => {
    ...
    const handleConfirmToggle = async () => {
        ...
        try {
            ...
            toast.success(`Account ${newStatus === "active" ? "unlocked" : "locked"} successfully!`);
        } catch (error) {
                ...
                <div className="bg-dark rounded-4 shadow-lg col-10 py-4 px-5">
                    ...
                    {accounts.length > itemsPerPage && (
                        <div className="pagination-container">
                            ...
                            {[...Array(Math.ceil(accounts.length / itemsPerPage)).keys()].map(number => (
                                <button
                                    key={number + 1}
                            ...
                            <button className="btn btn-outline-dark ms-2" onClick={() => paginate(currentPage + 1)} disabled={currentPage === Math.ceil(accounts.length / itemsPerPage)}>
                                &raquo;
                            </button>
                        </div>
                    )}
