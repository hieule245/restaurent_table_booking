...
const Admin = () => {
  ...
  const fetch = () => {
    try {
      axios
        .get(`${process.env.REACT_APP_API_URL}/admin/reservations`, {
  ...
  const handleUpdate = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_URL}/owners/:owner_id/reservations/finish_booking`,
        finish,
        { withCredentials: true }
      )
      .then(() => {
        ...
        <div className="bg-dark rounded-4 shadow-lg col-10 py-4 px-5">
          ...
          <div className="d-flex justify-content-between align-items-center w-100">
            <h1 className="text-danger fw-bold d-flex justify-content-center gap-2">
              <FaCalendarCheck className="pt-2" /> Revenues
            </h1>
          </div>
            ...
            <table className="table table-hover align-middle mb-0">
              <thead className="bg-danger text-white text-center">
                <tr>
                  ...
                  <th>
                    <FontAwesomeIcon icon={faStoreAlt} /> Restaurant
                  </th>
                  <th>
                    <FontAwesomeIcon icon={faCalendarAlt} /> Date
                  </th>
                  <th>
                    <FontAwesomeIcon icon={faClock} /> Booking
                  </th>
                  <th>
                    <FontAwesomeIcon icon={faClock} /> Arrival
                  </th>
                  <th>
                    <FontAwesomeIcon icon={faChair} /> Table
                  </th>
                  <th>Total</th>
                  <th>Status</th>
                </tr>
              ...
              <tbody className="text-center">
                {currentRes.map((res, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      {res.ActualTime || <span className="text-muted">--</span>}
                    </td>
                    <td>{res.TableName}</td>
                    <td className="text-danger fw-bold">
                      {res.Price.toLocaleString()} VND
                    </td>
                    <td>
                      <span
