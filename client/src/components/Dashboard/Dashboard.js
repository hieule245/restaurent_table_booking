...
const Dashboard = () => {
    ...
    const fetch = () => {
        try {
            axios
                .get(`http://localhost:8080/owners/:owner_id`, { withCredentials: true })
                .then((res) => {
    ...
    const topReservation = () => {
        try {
            axios
                .get(`http://localhost:8080/owners/:owner_id/top`, { withCredentials: true })
                .then((res) => {
    ...
    const chart = () => {
        try {
            axios
                .get(`http://localhost:8080/owners/:owner_id/static`, { withCredentials: true })
                .then((res) => {
                    console.log("API Response:", res.data); // Kiểm tra dữ liệu trả về
    ...
    return (
        <div className="container-fluid p-4 mt-3" style={{ background: "#F1F8FF" }}>
            ...
            <div className="row">
                {[
                    { title: "Revenue for this week", amount: revenues?.WeeklyRevenue ? revenues.WeeklyRevenue.toLocaleString() + " VND" : "0 VND", change: revenues.DiffTotal > 0 ? "+" + revenues.DiffTotal + "%" : revenues.DiffTotal < 0 ? "-" + revenues.DiffTotal + "%" : "", up: revenues.DiffTotal >= 0 ? true : false },
                    { title: "Booked and used tables", amount: revenues?.BookNumber ? revenues?.BookNumber : "0", change: revenues.DiffBookNumber > 0 ? "+" + revenues.DiffBookNumber + "%" : revenues.DiffBookNumber < 0 ? "-" + revenues.DiffBookNumber + "%" : "", up: revenues.DiffBookNumber >= 0 ? true : false },
                    { title: "Cancelled tables", amount: revenues?.CanceledBook ? revenues?.CanceledBook : "0", change: revenues.DiffCanceledBook > 0 ? "+" + revenues.DiffCanceledBook + "%" : revenues.DiffCanceledBook < 0 ? "-" + revenues.DiffCanceledBook + "%" : "", up: revenues.DiffCanceledBook >= 0 ? false : true }
                ].map((stat, index) => (
                    <div key={index} className="col-md-4 mb-3">
                        <div className="p-3 py-4 rounded shadow-sm"
                            style={{ backgroundColor: "#FFFFFF", border: "2px solid #D1E7FF", borderRadius: "12px" }}>
                            <div className="d-flex justify-content-between align-items-center">
                                ...
                                <div className="rounded-circle d-flex align-items-center justify-content-center shadow-sm"
                                    ...
                                    <FontAwesomeIcon icon={stat.up ? faArrowUp : faArrowDown}
                    ...
                    <div className="p-3 rounded shadow-sm w-100 h-100 d-flex flex-column"
                        ...
                        <div className="position-relative d-flex justify-content-center align-items-center flex-grow-1">
                            ...
                            <div className="position-absolute d-flex flex-column align-items-center" style={{ top: "42%" }}>
                                <h5 className="text-dark mb-0">{revenues.WeeklyRevenue ? revenues.WeeklyRevenue.toLocaleString() : 0}</h5>
