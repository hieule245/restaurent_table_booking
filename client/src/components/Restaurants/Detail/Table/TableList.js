...
const TableList = ({ restaurant_id }) => {
  ...
  const handleChange = (e) => {
    setFormData({
      ...formData,
      [e.target.name]:
        e.target.name === "seats" ? Number(e.target.value) : e.target.value,
    });
  };
  ...
  const fetchTables = async () => {
    try {
      const response = axios
        .get(
          `${REST_API_URL}/owners/:owner_id/restaurants/${restaurant_id}/tables`,
  ...
  useEffect(() => {
    axios
      .get(`${REST_API_URL}/owners/:owner_id/restaurants/${restaurant_id}`, {
        withCredentials: true,
      })
      .then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(
        `${REST_API_URL}/owners/:owner_id/restaurants/${restaurant_id}`,
        formData,
        { withCredentials: true }
      )
      .then(() => {
  ...
  const handleDeleteRestaurant = () => {
    axios
      .delete(`${REST_API_URL}/owners/:owner_id/restaurants/${restaurant_id}`, {
        withCredentials: true,
      })
      .then(() => {
  ...
  return (
    <div className="container-fluid row align-items-center">
        ...
        <div className="">
          <div className="restaurant-details mt-1 pt-1 pb-0">
            ...
            <p className="restaurant-hours d-flex align-items-center p-2 rounded-3 shadow-sm bg-light">
              ...
              <span className="fw-bold text-success px-2 py-1 bg-white rounded-3 border border-success">
                {restaurant.Started}
              </span>
              <span className="mx-2 text-muted">to</span>
              <span className="fw-bold text-danger px-2 py-1 bg-white rounded-3 border border-danger">
                {restaurant.Ended}
              </span>
            </p>
          </div>
      ...
      <div className="">
        ...
        {tables.length > tablesPerPage && (
          <div className="pagination-container m-0">
            ...
            {[...Array(Math.ceil(tables.length / tablesPerPage)).keys()].map(
              (number) => (
                <button
                  key={number + 1}
                  className={`btn ${
            ...
            <button
              ...
              onClick={() => paginate(currentPage + 1)}
              disabled={
                currentPage === Math.ceil(tables.length / tablesPerPage)
              }
            >
              &raquo;
            </button>
      ...
      <div
        ...
        tabIndex="-1"
        aria-labelledby="editRestaurantModalLabel"
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content rounded-4 shadow-lg border-0">
            <div className="modal-header bg-dark text-white rounded-top-4">
              <h4 className="modal-title fw-bold" id="editRestaurantModalLabel">
                Edit Restaurant
              </h4>
              <button
                type="button"
      ...
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content rounded-4 shadow-lg border-0">
            <div className="modal-header bg-dark text-white rounded-top-4">
              <h4 className="modal-title fw-bold" id="addTableModalLabel">
                Add Table
              </h4>
              <button
                type="button"
