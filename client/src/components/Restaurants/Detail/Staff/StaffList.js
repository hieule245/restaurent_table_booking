...
export default function StaffList({ restaurant_id }) {
  ...
  const fetchStaff = () => {
    axios
      .get(`${REST_API_URL}/owners/:owner_id/${restaurant_id}/staffs`, {
  ...
  const handleConfirmToggle = async () => {
    ...
    try {
      await axios.post(
        `${REST_API_URL}/owners/:owner_id/${restaurant_id}/staffs/${id}`,
        { Status: status, Id: id },
        { withCredentials: true }
      );

      setStaff((prevStaff) =>
        prevStaff.map((s) => (s.id === id ? { ...s, status: newStatus } : s))
      );
      toast.success(
        `Staff ${newStatus === "active" ? "unlocked" : "locked"} successfully!`
      );
    } catch (error) {
    ...
    <div className="container mt-5">
            ...
            <div key={id} className="col-md-3 mb-4">
              <div className="card w-100 h-100 shadow-lg border-2 border-danger rounded-4 bg-light text-dark position-relative p-3">
                <button
                  ...
                  onClick={() =>
                    status !== "ban" && handleOpenModal({ id, name, status })
                  }
                  disabled={status === "ban"}
                >
                  {status === "active" ? (
          ...
          <div className="modal-content rounded-4 shadow-lg border-0">
            ...
            <div className="modal-body p-4 bg-white">
              <p className="text-dark">
                ...
                <strong>
                  {selectedStaff?.status === "active" ? "lock" : "unlock"}{" "}
                </strong>
                staff{" "}
                <strong className="text-danger">{selectedStaff?.name}</strong>?
              </p>
            </div>
      ...
      {staff.length > itemsPerPage && (
        <div className="pagination-container">
          ...
          {[...Array(Math.ceil(staff.length / itemsPerPage)).keys()].map(
            (number) => (
              <button
                key={number + 1}
                className={`btn ${
          ...
          <button
            ...
            onClick={() => paginate(currentPage + 1)}
            disabled={currentPage === Math.ceil(staff.length / itemsPerPage)}
          >
            &raquo;
          </button>
        </div>
