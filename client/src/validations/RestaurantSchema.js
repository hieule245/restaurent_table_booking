...
const nameRegex = /^[A-Za-zÀ-ỹ\s]+$/; // Regex cho tên không chứa ký tự đặc biệt
...
function isSamePeriod(startMin, endMin) {
    ...
    const isMorning = time => time >= 0 && time < 720; // 00:00 - 11:59
    const isAfternoon = time => time >= 720 && time < 1440; // 12:00 - 23:59

    if ((isMorning(startMin) && isMorning(endMin)) ||
...
export const restaurantEditSchema = Yup.object().shape({
    ...
    Started: Yup.string()
        ...
        .test("valid-time-range", "Opened time must be before closed time.", function (value) {
    ...
    Ended: Yup.string()
        .required("Closed time is required."),

    Location: Yup.string()
        .required("Location is required.")
        .test("no-emoji", "Location must not contain emojis.", value => !emojiRegex.test(value || ""))
});
