...
const typeRegex = /^[a-zA-ZÀ-ỹ0-9 _-]+$/;
export const tableSchema = Yup.object().shape({
  ...
  type: Yup.string()
    ...
    .matches(typeRegex, {
      message: "Invalid the Type format .",
      excludeEmptyString: true,
    })
    .max(50, "The Type must not exceed 50 characters."),
  seats: Yup.number()
    .required("Seats is required")
    .positive("Seats must be a positive number")
    .min(1, "The seats must be at least 1 .")
    .integer("Seats must be an integer")
    .max(30, "The seats must be no more than 30"),
